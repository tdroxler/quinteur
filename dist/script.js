...
function noteToSemitone(note) {
    ...
    if (note.accidental === '##')
        semitone += 2;
    if (note.accidental === 'bb')
        semitone -= 2;
    return ((semitone % 12) + 12) % 12;
}
// Convert semitone + letter to note with accidental
function semitoneToNote(semitone, targetLetter) {
    const baseValues = {
        'C': 0, 'D': 2, 'E': 4, 'F': 5, 'G': 7, 'A': 9, 'B': 11
    };
    ...
    if (diff === 2)
        return { letter: targetLetter, accidental: '##' };
    if (diff === 11)
        return { letter: targetLetter, accidental: 'b' };
    if (diff === 10)
        return { letter: targetLetter, accidental: 'bb' };
    // Shouldn't happen for fifths
    return { letter: targetLetter, accidental: '' };
}
function calculateFifth(note, direction, quality) {
...
function generateQuestion() {
    ...
    const direction = Math.random() > 0.5 ? 'ascendante' : 'descendante';
...
class QuinteurGame {
    ...
    setupEventListeners() {
        ...
        this.accidentalButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                const accidental = btn.getAttribute('data-accidental');
    ...
    selectAccidental(accidental) {
        if (this.state.answered)
            return;
        console.log('Clicking accidental:', accidental);
        console.log('Currently selected:', this.selectedAccidental);
        // Toggle: if clicking the same accidental, deselect it
        if (this.selectedAccidental === accidental) {
            console.log('Deselecting accidental');
            this.selectedAccidental = '';
        }
        else {
            console.log('Selecting new accidental');
            this.selectedAccidental = accidental;
        }
        console.log('After selection:', this.selectedAccidental);
        // Update UI
        this.accidentalButtons.forEach(btn => {
            const btnAccidental = btn.getAttribute('data-accidental');
            if (btnAccidental === this.selectedAccidental) {
                btn.classList.add('selected');
                console.log('Adding selected to', btnAccidental);
            }
            else {
                btn.classList.remove('selected');
                console.log('Removing selected from', btnAccidental);
            }
        });
    }
    newQuestion() {
    ...
    checkAnswer() {
        if (this.state.answered || !this.state.currentQuestion || this.selectedLetter === null)
            return;
        ...
        if (!selectedFr) {
            console.error(`Unknown note: ${selectedStr}`);
